function strCell = num2cellstr(x, varargin )
...
%       prefix    - string to put befor number
%       suffix    - string to add after number
%
%
% Output parameters:
%
...
% Edited 21-11-2013 Manuel Pusch

%% read varargin
p=inputParser;
addParamValue(p,'format',[],@(x) ischar(x) || isempty(x));
addParamValue(p,'prefix',[],@(x) ischar(x) || isempty(x));
addParamValue(p,'suffix',[],@(x) ischar(x) || isempty(x));
parse(p,varargin{:});
...
% add prefix/suffix
strCell = cellfun(@(x) [prefix x suffix],strCell,'UniformOutput',false);
