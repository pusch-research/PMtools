...
%   SPY(S) plots the sparsity pattern of the matrix/ss S.
%
...
%   $Revision: 5.19.4.7 $  $Date: 2014/02/18 15:33:13 $ Manuel Pusch
% 
...
if nargin>=1 && isa(varargin{1},'ss')
    ...
    nx=length(S.A);
    varargin{1}=[S.A S.B;S.C S.D];
    spy_builtin(varargin{:});
    bHold=ishold;
    hold on;
    plot([nx nx]+0.5,[0 ny+nx+0.5],'g-','LineWidth',1);
    plot([0 nx+nu+0.5],[nx nx]+0.5,'g-','LineWidth',1);
    
    xlabel(['nx=' num2str(order(S)) ', nu=' num2str(nu) ' ,ny=' num2str(ny)]);
...
function []=spy_builtin(S,arg2,arg3)
%SPY Visualize sparsity pattern.
