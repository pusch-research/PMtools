...
%MATLABFRAG Exports figures to LaTeX using psfrag
...
%
% see also: INImatlabfrag PVmatlabfrag
%
% EXAMPLE
% plot(1:10,rand(1,10));
% set(gca,'FontSize',8);
...
if p.Results.debuglvl >= SHOW_OPTIONS
  ...
  if verLessThan('matlab', '8.3')%                                          % R2014b bugfix by knob_an
      fprintf(1,'OPTION: handle = %f\n',p.Results.handle);%                 % R2014b bugfix by knob_an
  else%                                                                     % R2014b bugfix by knob_an
      fprintf(1,'OPTION: handle = %f\n',p.Results.handle.Number);%          % R2014b bugfix by knob_an
  end%                                                                      % R2014b bugfix by knob_an
...
try
  ...
  for ii=1:size(PsfragCmds,1)
    ...
    if NewFontStyle
      ...
      if FontStyleId > double('Z')
          FontStyleId = double('A');
          FontStylePrefix = [FontStylePrefix 'A'];
      end
  ...
  function ProcessTicks(handle)
    ...
    for jj = ['x' 'y' 'z']
      ...
      if ~isempty(ticklabels)
        ...
        if ~TicksAreNumbers && isprop(handle,'TickLabelInterpreter') % 17-02-15 pusch.research@gmail.com
           if strcmpi(get(handle,'TickLabelInterpreter'),'tex')
                ...
                ticklabels   = regexprep(ticklabels, ['(\' implode('|\', specials) ')'], '$$1$');
                ticklabels   = regexprep(ticklabels, '(_|\^)({.*?}|.)', '$$1{\\mathrm{$2}}$');
           elseif strcmpi(get(handle,'TickLabelInterpreter'),'latex')
               error('ticklabelinterpreter latex is not supported.');
           end
        end
        if TicksAreNumbers
          if strcmpi(jj,'x')
    ...
    % legend 17-02-15 pusch.research@gmail.com
    hLegend=get(handle,'Legend');
    if ~isempty(hLegend)
        if strcmpi(get(hLegend,'Interpreter'),'none')
            legendStrings=get(hLegend,'String');
            legendStrings=cellfun(@(x)['$\mathrm{' x '}$'],legendStrings,'un',false);
            SetUnsetProperties('LegendStrings replacement',hLegend,'String',legendStrings); % manually set to latex
            SetUnsetProperties('LegendInterpreter replacement',hLegend,'Interpreter','Latex'); % manually set to latex
        elseif strcmpi(get(hLegend,'Interpreter'),'tex')
            specials = {'\alpha' '\upsilon' '\sim' '\beta' '\phi' '\leq' '\gamma' '\chi' '\infty' '\delta' '\psi' '\clubsuit' '\epsilon' '\omega' '\diamondsuit' '\zeta' '\Gamma' '\heartsuit' '\eta' '\Delta' '\spadesuit' '\theta' '\Theta' '\leftrightarrow' '\vartheta' '\Lambda' '\leftarrow' '\iota' '\Xi' '\uparrow' '\kappa' '\Pi' '\rightarrow' '\lambda' '\Sigma' '\downarrow' '\mu' '\Upsilon' '\circ' '\nu' '\Phi' '\pm' '\xi' '\Psi' '\geq' '\pi' '\Omega' '\propto' '\rho' '\forall' '\partial' '\sigma' '\exists' '\bullet' '\varsigma' '\ni' '\div' '\taut' '\cong' '\neq' '\equiv' '\approx' '\aleph' '\Im' '\Re' '\wp' '\otimes' '\oplus' '\oslash' '\cap' '\cup' '\supseteq' '\supset' '\subseteq' '\subset' '\int' '\in' '\o' '\rfloor' '\lceil' '\nabla' '\lfloor' '\cdot' '\ldots' '\perp' '\neg' '\prime' '\wedge' '\times' '\0' '\rceil' '\surd' '\mid' '\vee' '\varpi' '\copyright' '\langle' '\rangle'};
            specials = fliplr(sort(specials));  %#ok<FLPST> % Ensure \omega before \o...
            legendStrings=get(hLegend,'String');
            legendStrings   = regexprep(legendStrings, ['(\' implode('|\', specials) ')'], '$$1$');
            legendStrings   = regexprep(legendStrings, '(_|\^)({.*?}|.)', '$$1{\\mathrm{$2}}$');
            SetUnsetProperties('LegendStrings replacement',hLegend,'String',legendStrings); % manually set to latex
            SetUnsetProperties('LegendInterpreter replacement',hLegend,'Interpreter','latex'); % manually set to latex
        end  
    end
...
% magic to recombine them.
  function EpsCombine(handle,renderer,filename,dpiswitch,keep_tempfile)
    if verLessThan('matlab', '8.3')%                                        % R2014b bugfix by knob_an  
        ...
        TEXTOBJ_REGEXP = [...%                                              % R2014b bugfix by knob_an
            'GS\n', ...%                                                    % R2014b bugfix by knob_ab
            '\[[^\n]*\] CT\n', ...%                                         % R2014b bugfix by knob_ab
            '([^\n]* GC\n)?', ...%                                          % R2014b bugfix by knob_ab
            '\/[^\n]*\nGS\n', ...%                                          % R2014b bugfix by knob_ab
            '\[[^\n]*\] CT\n', ...%                                         % R2014b bugfix by knob_ab
            ...
            '\([^\n]*\) t \n', ...%                                         % R2014b bugfix by knob_ab
            'GR\n', ...%                                                    % R2014b bugfix by knob_ab
            'GR'];%                                                         % R2014b bugfix by knob_ab
        TEXTHDR_REGEXP = '';  %                                             % R2014b bugfix by knob_an      
    end%                                                                    % R2014b bugfix by knob_an
    if keep_tempfile
    ...
    % Insert the new text
    if verLessThan('matlab', '8.3')%                                        % R2014b bugfix by knob_an   
        findex = regexp(epsfile,'end %%Color Dict');%                       % R2014b bugfix by knob_an
    else%                                                                   % R2014b bugfix by knob_an
      	findex = regexp(epsfile, '%%Trailer');%                             % R2014b bugfix by knob_an
    end
    epsfile = sprintf('%s\n\n%s\n%s',...
      epsfile(1:findex-1),...
      sprintf('%s\n',textData{:,2}),...
