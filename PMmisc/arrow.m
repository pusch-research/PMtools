...
while (any(axnotdone)),
	...
	[azA,elA] = view(curax);
    curT = viewmtx(azA,elA); % curT = get(curax,'Xform'); pusch.research@gmail.com
...
%	% additional properties
	...
	if isempty(oldaxlims),
		...
		lims = get(ax(oldaxlims(:,1)),{'XLim','YLim','ZLim'})';% lims = get(oldaxlims(:,1),{'XLim','YLim','ZLim'})'; pusch.research@gmail.com
		lims = reshape(cat(2,lims{:}),6,size(lims,2));
		mask = arrow_is2DXY(ax(oldaxlims(:,1))); % mask = arrow_is2DXY(oldaxlims(:,1)); pusch.research@gmail.com
		oldaxlims(mask,6:7) = lims(5:6,mask)';
		ARROW_AXLIMITS = oldaxlims(find(any(oldaxlims(:,2:7)'~=lims)),:);
		if ~isempty(ARROW_AXLIMITS),
			%warning(arrow_warnlimits(ARROW_AXLIMITS,narrows));            % 16-04-25 pusch.research@gmail.com
		end;
	end;
else,
