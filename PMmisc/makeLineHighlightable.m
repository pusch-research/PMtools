...
% Revisions: 11/13/07- Other lines turn off when line is highlighted
    ...
    if strcmp('figure',get(h,'type'))
        ax_arr = findall(h,'type','axes');
        for ax_act=ax_arr(:)'
            makeLineHighlightable(ax_act,highlightPoints);
        end
        return
    end
...
%     if isempty(lineHdls)
%         warning('makeLineHighlightable:noInputLineHandles','There are no line handles in input array.');
%     end        
end %parseHandles
