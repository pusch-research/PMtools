function dcmH = customDataCursor(h, datalabels,varargin)
...
elseif numel(datalabels) ~= numel(get(h,'xdata'))
...
set(dcmH,'UpdateFcn',[{@cursorUpdateFcn},varargin])

function output_txt = cursorUpdateFcn(obj,event_obj,varargin)
...
%     for ii=1:2:nargin-2
%         set(hText,varargin{ii},varargin{ii+1});            % Change the interpreter
%     end
end
...
if ~isempty(labels)
    ...
    datapoint = (xvals==pos(1))&(yvals==pos(2));

    % create the text to be displayed
    output_txt = [ labels{find(datapoint,1)}(:);...
        {['X: ',num2str(pos(1),4)]};...
        {['Y: ',num2str(pos(2),4)]}];
else
    % create the text to be displayed
