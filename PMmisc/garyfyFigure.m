...
if ischar(fh) && strcmp(fh,'all')
    ...
    return;
elseif (isnumeric(fh) || isa(fh,'matlab.ui.Figure')) && isvector(fh) && ~isscalar(fh)
    % multiple figure handles, call this function recursively
    for k=1:numel(fh); garyfyFigure(fh(k),opts); end
    return;
elseif (isnumeric(fh) || isa(fh,'matlab.ui.Figure')) && isscalar(fh)
...
% keyboard
h = findall(fh,'Type','axes'); % 'Visible','on' % 18-01-31 pusch.research@gmail.com
...
for k=1:numel(h)
    ...
    sel_iArr=arrayfun(@(x) ~isequal(x{:},'CSTgridLines'),tag_arr); % from sgrid.m
    set(h2(sel_iArr),'LineWidth',opts.LineWidth);  % modify line width
