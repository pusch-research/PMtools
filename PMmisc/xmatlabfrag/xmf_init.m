...
% REVISIONS:    2011-07-05 First version (kloe_ad)
%               2011-07-07 Defaults for xmf_figure and xmf_subplot added (knob_an)
%               2012-07-11 Defaults xmf_subplot added (M.A.)
%
...
elseif nargin < 2
    ...
    % Issue warning for bad command
        ...
        warning('DLR:xmf_init:BadCommand', 'Bad command used for xmf_init: ''%s''.', varargin{1});
    end
...
%% Loop all settings passed to here
    ...
    for i_arg = 1:2:nargin-1
        ...
        % ... else issue warning
           ...
           warning('DLR:xmf_init:BadKey', 'Bad key used for xmf_init: ''%s''.', key);
        end
    end
end
